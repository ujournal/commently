<turbo-frame id="tags-frame">
    <ul class="nav-list">
        <li class="nav-list-item">
            <a href="{{ $url->to('forum')->route('custom-frontend.index') }}" class="nav-list-link nav-list-link--home" data-turbo-frame="_top">
                <div class="nav-list-link-icon nav-list-link-icon--home">
        ...
        @foreach ($primaryTags as $tag)
            ...
            <li class="nav-list-item">
                <a href="{{ $indexUrl }}" class="nav-list-link {{ $hasNew ? 'nav-list-link--has-new' : '' }}" data-turbo-frame="_top">
                    <div class="nav-list-link-icon" style="--url: url('/assets/topics/{{ $tag['attributes']['icon'] }}.png');"></div>
                    <span class="nav-list-link-text">{{ $name }}</span>
                    @if ($hasNew)
